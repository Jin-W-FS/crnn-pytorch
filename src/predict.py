...
def predict(crnn, dataloader, decode_method, beam_size):
    crnn.eval()
    ...
    with torch.no_grad():
        for data in dataloader:
            ...
            all_preds.extend(LPRDataset.decode(pred) for pred in preds)
...
def main():
    ...
    predict_dataset = LPRDataset(images, mode='predict', img_shape=img_shape)
    ...
    preds = predict(crnn, predict_loader,
                    decode_method=decode_method,
                    beam_size=beam_size)

    show_result(predict_dataset.img_paths, preds)
